...
do
   ...
   zcat ${INPUT} | awk 'BEGIN{FS="\t"; OFS="\t"}{if ($0~/MULTI_ALLELIC/) next; vt=""; info=$8; n=split(info,a,";"); for (i=1; i<=n; ++i) { split(a[i],b,"="); if (a[i]~/^VT=/) vt=b[2]; if (a[i]~/^AC=/) ac=b[2]; if (a[i]~/^AN=/) an=b[2]; }; if (vt=="SNP") {print "GLOBAL_AC="ac}}' >  ${PREFIX}/${CHR}/${CHR}_${GROUP}_global.ac
   zcat ${INPUT} | awk 'BEGIN{FS="\t"; OFS="\t"}{if ($0~/MULTI_ALLELIC/) next; vt=""; info=$8; n=split(info,a,";"); for (i=1; i<=n; ++i) { split(a[i],b,"="); if (a[i]~/^VT=/) vt=b[2]; if (a[i]~/^AC=/) ac=b[2]; if (a[i]~/^AN=/) an=b[2]; }; if (vt=="SNP") {print "GLOBAL_AN="an}}' > ${PREFIX}/${CHR}/${CHR}_${GROUP}_global.an
